...
- **What went wrong:** While testing the new `/api/onboard/finalize` endpoint, sent a curl POST with `{"basics":{"full_name":"Test User",...}}` to a server bound to PORT=4748 but with `ROOT` still pointing at the real project. The endpoint did exactly what it was designed to do — wrote `config/profile.yml` with the test payload. Tony's real profile (target_roles, archetypes, narrative, comp, deal-breakers) was destroyed. `profile.yml` is gitignored so `git checkout` could not restore it. Reconstructed from session memory (`memory/user_tony_profile.md`) but original archetype expansions and any post-memory updates may have been lost.
