<script lang="ts">
	...
	// Fetch full report markdown when offer changes
	$effect(() => {
...
<div class="panel" style="flex:1">
	{#if !offer}
		<div class="panel-header"><span class="title">Detail</span></div>
		<div style="padding:40px 24px;color:var(--fg-3);font-family:var(--mono);font-size:12px">
			↑↓ select a role from the pipeline.
		</div>
	{:else}
		<div class="panel-header">
			<span style="color:var(--accent)">#{offer.n}</span>
			...
			<div class="right">
				...
				{#if offer.url}
					<a href={offer.url} target="_blank" rel="noopener" class="icon-btn" title="Open job posting">↗</a>
				{/if}
				<button class="icon-btn" title={offer.state === 'applied' ? 'Undo applied' : 'Mark applied'} onclick={() => changeState(offer.state === 'applied' ? 'evaluated' : 'applied')}>a</button>
