          ...
          {activeTab === 'dashboard' && (
            <motion.div key="dash" className="space-y-10">
               ...
               <div className="grid grid-cols-2 gap-10">
                  <div className="bg-white/5 p-8 rounded-3xl border border-white/10 aspect-video flex flex-col justify-between text-white/80">
                     <div>
                       <h3 className="text-2xl font-bold mb-2 text-white">Application Funnel</h3>
            ...
            <motion.div key="terminal" className="bg-black border border-white/10 rounded-2xl flex flex-col h-[500px] overflow-hidden shadow-2xl relative">
              <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/[0.02] z-10">
                 ...
                 <div className="flex gap-4 items-center">
                  ...
                  <div className="flex gap-2 p-3 bg-white/[0.03] border-t border-white/5 overflow-x-auto no-scrollbar font-mono">
                     <button onClick={() => runCommand('scan')} className="px-3 py-1 rounded bg-amber-500/10 text-amber-500 text-[10px] font-bold border border-amber-500/20 whitespace-nowrap">SCAN</button>
                     <button onClick={() => runCommand('offer-list')} className="px-3 py-1 rounded bg-amber-500/10 text-amber-500 text-[10px] font-bold border border-amber-500/20 whitespace-nowrap">OFFER-LIST</button>
                     <button onClick={() => runCommand('apply')} className="px-3 py-1 rounded bg-emerald-500/10 text-emerald-500 text-[10px] font-bold border border-emerald-500/20 whitespace-nowrap">APPLY</button>
                     <button onClick={() => runCommand('help')} className="px-3 py-1 rounded bg-white/5 text-white/40 text-[10px] font-bold border border-white/10 whitespace-nowrap">HELP</button>
                  </div>

                  <div className="p-4 bg-[#050505]">
                     <div className="flex items-center gap-2 group">
                        <span className="text-amber-500 font-bold shrink-0 font-mono">career-ops &gt;</span>
                        <form onSubmit={handleCommandSubmit} className="flex-1">
                           <input 
                             ...
                             placeholder={isExecuting ? 'Waiting for process...' : 'scan · apply <id> · help'}
                             disabled={isExecuting}
                             className="w-full bg-transparent outline-none border-none text-[#fafafa] placeholder:text-white/10 caret-amber-500 font-mono"
                           />
                        </form>
                     </div>
                  </div>
          ...
          {activeTab === 'settings' && (
            <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex-1 overflow-y-auto w-full max-w-5xl">
               <div className="flex justify-between items-start mb-8">
                 <div>
                   <h2 className="text-3xl font-bold mb-2 text-white">Professional Profile</h2>
                   <p className="text-white/40">Configure your global identity for AI job discovery.</p>
                 </div>
                 <button 
                 ...
                 >
                   {saveStatus === 'saving' ? 'Syncing...' : saveStatus === 'success' ? <><CheckCircle2 size={18} /> Saved</> : 'Save Changes'}
                 </button>
               </div>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 text-white/80">
                 <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white"><LayoutDashboard size={18} className="text-blue-500" /> Identity</h3>
                    <div className="space-y-4">
                       <Input label="Full Name" value={profileFormData.candidate.full_name} onChange={(v) => setProfileFormData({...profileFormData, candidate: {...profileFormData.candidate, full_name: v}})} placeholder="John Doe" />
                       <div className="grid grid-cols-2 gap-4">
                          <Input label="Location" value={profileFormData.candidate.location} onChange={(v) => setProfileFormData({...profileFormData, candidate: {...profileFormData.candidate, location: v}})} placeholder="Berlin" />
                          <Input label="Email" value={profileFormData.candidate.email} onChange={(v) => setProfileFormData({...profileFormData, candidate: {...profileFormData.candidate, email: v}})} placeholder="john@example.com" />
                       </div>
                    </div>
                 </div>

                 <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white"><FileText size={18} className="text-amber-500" /> Narrative</h3>
                    <div className="space-y-4">
                       <Input label="Professional Headline" value={profileFormData.narrative.headline} onChange={(v) => setProfileFormData({...profileFormData, narrative: {...profileFormData.narrative, headline: v}})} placeholder="Software Engineer" />
                       <div>
                          <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-1.5 block">Summary</label>
                          <textarea 
                            rows={4}
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:border-amber-500/50 transition-all resize-none text-sm" 
                          />
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white"><Search size={18} className="text-emerald-500" /> Filter</h3>
                    <div className="space-y-4">
                       <Input label="Target Keywords" value={profileFormData.targeting_keywords.positive.join(', ')} onChange={(v) => setProfileFormData({...profileFormData, targeting_keywords: {...profileFormData.targeting_keywords, positive: v.split(',').map(s => s.trim())}})} placeholder="Remote, AI" />
                    </div>
                 </div>

                 <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white"><ShieldCheck size={18} className="text-purple-500" /> Secrets</h3>
                    <div className="space-y-4">
                       <Input label="OpenAI API Key" type="password" value={profileFormData.openai_key} onChange={(v) => setProfileFormData({...profileFormData, openai_key: v})} placeholder="sk-..." />
                    </div>
                 </div>
               </div>
            </motion.div>
  ...
  return (
    <div>
      ...
      <input 
        ...
        className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:border-white/30 transition-all text-sm" 
